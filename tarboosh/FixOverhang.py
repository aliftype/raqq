...
The other verb, ``FixOverhang`` deals with the problem of short bari ye
...
``FixOverhang 10 @bariye`` will ensure that there are at least 10 points
between the end of the tail of each glyph in ``@bariye`` and any isolated/final
glyph preceding the sequence.

``FixOverhang`` can also take an optional integer value for the adjustment
threshold, any adjustments less than the threshold will be dropped. e.g.
``FixOverhang 10 100 @bariye`` will drop any adjustments that are less than
100 units.

``FixOverhang`` can optionally take initials and medials glyph classes
directly, e.g. ``FixOverhang 10 @bariye @medials @initials``.
